...
const getaProduct = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const color_ = await Promise.all(findProduct.color.map(async (i) => {
      const color = await Color.findById(i);
      return { color: color.title, _id: color._id };
    }));
    findProduct.color = color_;
